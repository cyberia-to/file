...
/// EBML container (magic `1A 45 DF A3`): the header DocType tells WebM
/// and Matroska apart, since both share the same container format.
fn sniff_ebml(data: &[u8]) -> Option<Kind> {
    ...
    while pos < header_end {
        ...
        if id == 0x4282 {
            // DocType
            return match &data[pos..end] {
                b"webm" => Some(Kind::VideoWebm),
                b"matroska" => Some(Kind::VideoMkv),
                _ => None,
            };
        }
        pos = end;
...
mod tests {
    ...
    #[test]
    fn matroska_doctype_sniffs_as_video_mkv() {
        assert_eq!(sniff(&ebml(b"matroska")), Kind::VideoMkv);
    }

    #[test]
    fn unknown_ebml_doctype_is_opaque() {
        assert_eq!(sniff(&ebml(b"quicktime")), Kind::Opaque);
    }
